...
st.set_page_config(
    page_title="SmartLottoAI",
    page_icon="🎱",
    layout="wide"
)

st.title("🎱 SmartLottoAI")
...
if menu == "홈":
    ...
    st.success("SmartLottoAI 개발 진행 중")

    st.write("✅ 엑셀 통합")
    st.write("✅ 파일 분석")
    st.write("✅ 시트 분석")
    st.write("🔄 데이터베이스 구축 예정")
...
if menu == "로또DB":
    ...
    try:
        ...
        freq_df["번호"] = freq_df["번호"].astype(int)
        ...
        st.subheader("🏆 출현 TOP10")

        top10 = freq_df.sort_values(
            "출현횟수",
            ascending=False
        ).head(10)
